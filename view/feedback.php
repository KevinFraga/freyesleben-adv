...
<main class="page-container">
  <div class="title-display">
    <p class="title">Depoimentos dos Clientes</p>
    <p class="subtitle">O que andam falando de nós...</p>
  </div>

  <?php
  if (isset($error_feedback)) {
    ?> 
    <div class="title-display">
      <p class="form-error"><?= $error_feedback?></p>
    </div>
    <?php
  }
  ...
  $query = mysqli_query($connect2, "SELECT id, user_id, title, text, DATE_FORMAT(created_at, '%d.%m.%y  %H:%i') AS created_at, DATE_FORMAT(updated_at, '%d.%m.%y  %H:%i') AS updated_at FROM JohnnyBravo ORDER BY created_at DESC;");
    ...
    <div class="post">
      ...
      if (isset($_SESSION['user_id']) && ($row['user_id'] == $_SESSION['user_id'] || $_SESSION['user_kind'] == 'adv')) {
        ...
        <div class="partner-actions">
          <div>
            <a class="p-step" href="<?= "/depoimentos/editar?id={$_SESSION['user_id']}&post={$row['id']}" ?>">
              <img class="icon" src="/view/images/pencil-icon.png" alt="editar" />
            </a>
          </div>
          <div>
            <form action="<?php htmlspecialchars($_SERVER['PHP_SELF']) ?>" method="post" onsubmit="return confirm('Tem certeza que deseja deletar esse depoimento?');">
  ...
  if (isset($_SESSION['user_id'])) {
    ...
    <div class="func-row">
      ...
      <div class="func-col">
        <a href="<?= "/depoimentos/novo?id={$_SESSION['user_id']}" ?>">
          <span class="arrow">Adicionar Depoimento</span>
