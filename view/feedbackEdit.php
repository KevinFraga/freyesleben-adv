...
$query = mysqli_prepare($connect, "SELECT user_id, title, text FROM JohnnyBravo WHERE id = ?;");
...
if (isset($author) && ($author == $_SESSION['user_id'] || $_SESSION['user_kind'] == 'adv')) {
  ...
  <main class="page-container">
    ...
    <?php
    if (isset($error_edit_feed)) {
      ?> 
      <div class="title-display">
        <p class="form-error"><?= $error_edit_feed?></p>
      </div>
      <?php
    }
    ?>
    ...
    <div class="form">
      <form action="<?php htmlspecialchars($_SERVER['PHP_SELF']) ?>" method="post">
        <div>
          ...
          <input id="feed-title" name="title" type="text" value="<?= $title ?>" />
        </div>
        <div>
          <label for="feed-text" class="label">Texto</label>
          <textarea id="feed-text" name="text"><?= $text ?></textarea>
        </div>
